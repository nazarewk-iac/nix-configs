# The `development/rpi` module of the old tree, as a den aspect.
#
# The old module stays in place and keeps working. This file is the parallel den implementation.
#
# ## What it does
#
# Nothing yet. The one package it wants, `rpi-imager`, does not build. The file keeps the place, so
# the den tree holds one aspect per old module, and so the package comes back in one line.
#
...
let
  packages =
    pkgs: with pkgs; [
      #rpi-imager # TODO: 2025-10-28: it is broken. See nixpkgs issue 454826.
    ];
...
in
{
  kdn.dev-rpi.nixos = systemTarget;
  kdn.dev-rpi.darwin = systemTarget;
  kdn.dev-rpi.homeManager = homeTarget;
}
