...
in
{
  config = kdnConfig.util.ifTypes [ "nixos" ] (
    lib.pipe kdnConfig.k8s.clusters [
      builtins.attrValues
      ...
      (map (clusterCfg: {
        ...
        services.keepalived.vrrpInstances =
          let
            baseCfg = {
              state = if clusterCfg.apiserver.vrrp.masterNode == kdnConfig.hostName then "MASTER" else "BACKUP";
              interface = clusterCfg.apiserver.interface;
              virtualRouterId = clusterCfg.apiserver.vrrp.id;
              priority = if clusterCfg.apiserver.vrrp.masterNode == kdnConfig.hostName then 101 else 100;
