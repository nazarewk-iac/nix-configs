...
in
{
  options.kdn.services.k8s = {
    enable = lib.mkOption {
      type = lib.types.bool;
      default = builtins.any (clusterCfg: clusterCfg.isMember) (
        builtins.attrValues kdnConfig.k8s.clusters
      );
    };
      ...
      assertions = [
        {
          assertion = lib.pipe kdnConfig.k8s.clusters [
            ...
            (map (
              clusterCfg:
              builtins.any (
                allowed: lib.hasPrefix allowed cfg.packages.default.version
