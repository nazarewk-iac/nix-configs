...
{
  config =
    lib.mkIf (config.kdn.profile.machine.baseline.enable && config.kdn.security.secrets.allowed)
      (
        lib.mkMerge [
          {
              ...
              overrides = [
                (
                  ...
                  let
                    ...
                    result = old // {
                      mode =
                        if isPubKey filename old then
                          "0444"
                        else if isPrivKey filename old then
                          "0400"
                        else
                          "0440";
                    };
