...
in
{
  ...
  config = lib.mkIf (config.kdn.security.secrets.allowed) (
    lib.mkMerge [
      (lib.mkIf (cfg.server.enable) {
        ...
        systemd.services.iperf3.serviceConfig.LoadCredential = [
          ...
          "users.csv:${
            config.kdn.security.secrets.sops.secrets.default.networking.iperf-server."users.csv".path
          }"
        ];
        ...
        environment.systemPackages = [
          (pkgs.writeShellApplication {
            ...
            text =
              let
                defaultUsername =
                  lib.pipe config.kdn.security.secrets.sops.secrets.default.networking.iperf-server.users
                    [
                      builtins.attrNames
                      builtins.head
                    ];
