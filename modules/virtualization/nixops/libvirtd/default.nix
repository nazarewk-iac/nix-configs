...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    system.activationScripts.kdnLibVirtdInit =
      let
        virsh = "${config.virtualisation.libvirtd.package}/bin/virsh";
      in
      ''
        function kdnLibVirtdInit() {
          local libvirt=/var/lib/libvirt
          local libvirt_pool=default
          local libvirt_images="$libvirt/images"

          mkdir -p "$libvirt_images"
          chgrp -R libvirtd "$libvirt"
          chmod -R g+w "$libvirt"

          if ! ${virsh} pool-list >/dev/null ; then
            return 0
          fi
