...
in {
  config = lib.mkMerge [
    {home-manager.sharedModules = [{kdn.hardware.disks.enable = cfg.enable;}];}
    {
      ...
      kdn.hardware.disks.persist = lib.pipe cfg.base [
      ...
      preservation.preserveAt = lib.pipe cfg.base [
        ...
        in {
          ...
          users =
            builtins.mapAttrs (username: hmConfig: let
              ...
              process = topName: childName: mode:
                lib.pipe [
                  (hmConfig.kdn.hardware.disks.persist."${persistName}" or {})
                  ...
                  in
                    ...
                    // d
                    // {
                      parent = {mode = homeDirMode;} // (d.parent or {});
                    }))
                ];
            in {
    ...
    }
    {
      ...
      systemd.services = lib.pipe config.home-manager.users [
        ...
        (lib.attrsets.mapAttrs' (_: hmConfig: {
    ...
    in {
      ...
      */
      kdn.hardware.disks.base."disposable".snapshots = false;
      kdn.hardware.disks.base."disposable".zfsName = cfg.disposable.zfsName;
      kdn.hardware.disks.base."disposable".disko.postCreateHook = ''
        zfs snapshot "${snapshotName}"
      '';
      kdn.hardware.disks.persist."disposable".directories = [
        {
          directory = "/var/tmp";
          mode = "0777";
        }
    ...
    })
    {
      ...
      preservation.preserveAt = lib.pipe cfg.users [
        (lib.attrsets.mapAttrsToList (username: userCfg: let
