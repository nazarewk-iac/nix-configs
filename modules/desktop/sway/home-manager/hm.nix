  ...
  config = lib.mkIf (config.kdn.headless.enableGUI && cfg.enable) (lib.mkMerge [
    ...
    }
    {
      ...
      services.kanshi.settings =
        let
          pos = w: h: "${builtins.toString (builtins.floor w)},${builtins.toJSON (builtins.floor h)}";
          pos2 = devs: fn:
            let
              result = fn {
                x = builtins.mapAttrs (_: cfg: (recalc cfg).width) devs;
                y = builtins.mapAttrs (_: cfg: (recalc cfg).height) devs;
              };
            in
            "${builtins.toString (builtins.floor result.x)},${builtins.toJSON (builtins.floor result.y)}";
          ...
          devices = builtins.mapAttrs
              ...
              recalc (cfg // parsed))
            {
              ...
              asus-pg78q = {
                # it started reporting own name when connected to laptop o.O
          ...
          profiles = {
            desktop-full = {
              outputs = with devices; [
                {
                  inherit (asus-pg78q) criteria;
                  position = pos 0 0;
                  mode = "3840x2160@165Hz";
                }
                {
                  inherit (gb-m32uc) criteria;
                  position = pos asus-pg78q.width 0;
                }
              ];
            };
            oams = {
            ...
            oams-desktop-full = {
              outputs = with devices;   [
                ...
                {
                  inherit (oams) criteria;
                  position = pos (asus-pg78q.width + gb-m32uc.width) (gb-m32uc.height - oams.height);
                }
              ];
            };
            oams-desktop-m32uc = {
              outputs = with devices; [
                ...
                {
                  inherit (oams) criteria;
                  position = pos (gb-m32uc.width) (gb-m32uc.height - oams.height);
                }
              ];
            };
            oams-desktop-pg78q = {
    ...
    (
      let nwg = config.services.nwg-shell;
      in
      lib.mkMerge [
        {
