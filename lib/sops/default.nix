...
let
  parseSopsYAMLMetadata =
        ...
        let
          collector =
            ...
            let
              ...
              update =
                if cur.type != null && (builtins.head parents).key != "sops" then
                  { result = state.result ++ [ curResult ]; }
                else
                  { inherit parents; };
    ...
    lib.pipe file [
      (
        file:
        lib.trivial.throwIfNot (lib.strings.hasSuffix ".sops.yaml" file)
          "Can only parse '*.sops.yaml' files!"
          file
      )
      builtins.readFile
