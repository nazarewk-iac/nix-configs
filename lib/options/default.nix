    ...
    lib.types.submodule (args: {
      ...
      options.final = lib.mkOption {
        ...
        default = lib.pipe args.config.prev [
          (pkg: pkg.override (prev: lib.lists.foldl (old: fn: fn old) prev args.config.overrides))
          (pkg: pkg.overrideAttrs (prev: lib.lists.foldl (old: fn: fn old) prev args.config.overrideAttrs))
        ];
      };
    });
  mkOverridablePackageOption =
    pkg: args: lib.mkOption ({ type = overridablePackageType pkg; } // args);
in
{
  inherit
    overridablePackageType
