...
Goal: `docs/nix-darwin-getting-started.md`, `type: How-To`. An adopter with a stock Mac follows it,
then reaches a successful `x86_64-linux` build.

## Scope

Zero → nix-darwin installed → a minimal configuration that switches → the Rosetta builder active
...
may be on CppNix or Determinate Nix instead. Name where behaviour differs. Two Lix-on-Darwin facts,
both verified in this repo, belong in that note:
...
- Lix lacks `builtins.convertHash`. That broke a Homebrew-related overlay here. The creator then
  dropped the inputs. An adopter with such an overlay needs to know.
...
the first time. That image is an `aarch64-linux` derivation. So the order is:
...
- **Two builders at once are fragile.** Both cached builder images default to
  ...
  `--option sandbox false`)". It cites nix-darwin#1192.
...
- Rosetta 2 installed, with the Linux runtime. Check that
...
- The adopter must set `system.primaryUser`. nix-darwin activation fails without it.
...
| Symptom | Cause | Fix |
|---|---|---|
| You cannot build the customized Linux builder on a fresh Mac | Chicken-and-egg: the customization is itself a Linux derivation | First switch uses the stock cached builder; gate customization behind a toggle (`hosts/anji/default.nix`) |
| Error about "not being on linux and that substitutes are not allowed" during a Linux `toplevel` build from Darwin | Nix refuses to substitute the outer derivation because the local system cannot build it | Pass `--always-allow-substitutes` (`packages/darwin-rebuild/darwin-rebuild.sh`) |
| `/nix/store/…-stdenv-linux/setup: line 1828: wrapProgram: command not found` | A build of the customized Linux builder VM from Darwin | **Unresolved** in this repo — an open TODO above `nix.buildMachines` in `hosts/anji/default.nix`. Present it as a known trap, not a solved one |
...
| Host eval breaks on `nixpkgs.hostPlatform` plus `darwinSystem { system = …; }` | nix-darwin moved to `nixpkgs.system` with `system = null` | `flake.nix:328-336` shows the shape that works |
| `homebrew.taps` conflicts with `nix-homebrew.taps`; untrusted tap sources block installs | nix-homebrew manages `homebrew/*` taps itself | Filter taps by the `homebrew/` prefix **and** `trusted = true`. This repo took three iterations, and tracks nix-homebrew#128 |
...
| Build fails with "does not support your platform" from a Linux-only Home Manager module | The tree imports a Linux-only HM module unconditionally into a Darwin host's HM child | Guard it by parent type (`modules/universal/_stylix.nix`) |
...
| Eval aborts on Linux-only or broken packages in a shared package list | The repo shares one package list across platforms | This repo filters `meta.broken`, `meta.unsupported`, `!meta.available`, and `tryEval` failures with a warning, in `modules/universal/env/default.nix` |
...
Two more facts to state, both from `docs/multi-arch-builder.md`:

- A `"x86_64-linux"` entry in `systems` does nothing **without** the binfmt line (nix-darwin#1192).
...
Set expectations on time. A full NixOS `toplevel` through the Rosetta builder took about
**22 minutes warm** here. Compare about **32 minutes warm** on a real Linux desktop, and about
**6 hours cold**.
...
- A need to install Rosetta 2 — here it was already present.
- SIP, `/nix` volume creation, or a macOS upgrade that wipes `/etc/synthetic.conf` or the nix
  volume. This repo's Darwin host survived a macOS major upgrade with no recorded volume damage.
...
- That sudo with TouchID survives a rebuild. This repo configures it, but nobody recorded a
  failure. So the claim is generic.

## Exit criteria

- A reader who follows only the runbook, on a stock Mac, builds an `x86_64-linux` derivation.
- Every caveats row has evidence in this repo, or carries the unverified mark.
- No fork-only path appears — see the constraints section in the hub.
